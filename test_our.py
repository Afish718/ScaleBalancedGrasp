...
# from graspnet_object_balance_sampling import GraspNet,pred_decode
...
TEST_DATASET = GraspNetDataset(cfgs.dataset_root, valid_obj_idxs=None,
                               grasp_labels=None, split='test',
                               camera=cfgs.camera,
                               num_points=cfgs.num_point, remove_outlier=True,
...
# Init the model
net = GraspNet_MSCQ(input_feature_dim=0, num_view=cfgs.num_view, num_angle=12,
                    num_depth=4, cylinder_radius=0.08, hmin=-0.02,
...
def inference():
    ...
    for batch_idx, batch_data in enumerate(TEST_DATALOADER):
        ...
        with torch.no_grad():
            print("batch_data: \n", batch_data.keys())
            print("objectness_label: \n", batch_data["objectness_label"])
            batch_data["objectness_label"] = 0
            print("instance_mask: \n", batch_data["instance_mask"])
            batch_data["instance_mask"] = 0
            print("point_clouds: \n", batch_data["point_clouds"].size())
            print("cloud_colors: \n", batch_data["cloud_colors"].size())
            batch_data["cloud_colors"] = 0
            end_points = net(batch_data)

            """
            batch_data是一个词典
            dict_keys(['point_clouds', 'cloud_colors', 'instance_mask', 'objectness_label'])
            后面的net只是用到了point_clouds, 这里只需要输入point_clouds既可以了
            """
        ...
        for i in range(cfgs.batch_size):
            ...
            save_dir = os.path.join(
                cfgs.dump_dir, SCENE_LIST[data_idx], cfgs.camera)
